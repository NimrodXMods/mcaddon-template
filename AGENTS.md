...
| `packs/BP/manifest.json` changed after a build | `mct exportaddon` resolves the latest registry version of each script module dependency and writes it back as an **array** (`[2, 9, 0]`) where a semver **string** (`"2.0.0"`) is correct for `module_name` deps. Arrays are only right for pack-UUID deps. It also strips the trailing newline. `scripts/deploy.sh` snapshots and restores the manifests; after a hand-run `exportaddon`, check `git diff packs/`. Only bites when the registry is reachable, which makes it look intermittent - but **`--offline` does not prevent it**: confirmed 2026-08-31, an `--offline` export still rewrote both `@minecraft/server` and `@minecraft/server-ui` to arrays and stripped the newline. The `--offline` comment in `deploy.sh` promises a byte-identical *archive*, not an untouched *source*. |
