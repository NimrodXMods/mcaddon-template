...
- `mct exportaddon` rewrites script module dependency versions in
  `packs/BP/manifest.json`, normalizing strings to arrays. See the gotcha table;
...
| `packs/BP/manifest.json` changed after a build | `mct exportaddon` rewrites `module_name` dependency versions from a semver **string** (`"2.9.0"`) to an **array** (`[2, 9, 0]`), and strips the trailing newline. **This is normalization, not corruption** - both forms are valid. mct's own `IAddonDependency` types `version` as `number[] | string`, and `setModuleVersion()` converts stable 3-part versions to arrays while leaving anything containing `-` (i.e. `-beta`) a string. Prefer strings anyway: Learn's `@minecraft/server` page gives `{"module_name": "@minecraft/server", "version": "2.9.0"}` as canonical, and **manifest v3 requires a string** ("in version 3 ... you must use a string for version"), so the array runs against where the platform is heading - besides churning every diff. `--offline` does **not** prevent it (confirmed 2026-08-31); deploy.sh's `--offline` comment promises a byte-identical *archive*, not an untouched *source*. deploy.sh snapshots and restores either way; after a hand-run `exportaddon`, check `git diff packs/`. The trailing-newline strip has no upside and is a plain defect. |
